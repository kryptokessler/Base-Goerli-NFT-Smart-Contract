...
contract AccEth is ERC721, ERC721URIStorage {
    ...
    /* Updates the listing price of the contract */
    function updateResellingPrice(uint256 _resellingPrice) public payable {
        require(
            owner == msg.sender,
            "Only marketplace owner can update reselling price."
        );
    ...
    function updateContractOwner() public payable {
        require(
            owner == msg.sender,
            "Only marketplace owner can update marketplace owner"
        );
        owner = payable(msg.sender);
    }
    ...
    function resellToken(uint256 tokenId, uint256 price) public payable {
        ...
        require(
            msg.value == resellingPrice,
            "Price must be equal to reselling price"
        );
    ...
    function fetchEventNFTs(uint256 eventId)
        public
        view
    ...
    function fetchFreeEventNFTs(uint256 eventId)
        public
        view
        returns (MarketItem[] memory)
    {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (
                idToMarketItem[i + 1].eventId == eventId &&
